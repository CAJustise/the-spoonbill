/*
  ...
  1. New Tables
    ...
    - event_bookings: Special event bookings
    - blackout_dates: Dates when reservations/events are blocked
  
  2. Enums
    - table_status: Status of tables (available, reserved, etc)
    - reservation_status: Status of reservations
    - event_booking_status: Status of event bookings
  
  3. Functions
    - check_table_availability: Check if a table is available
    - check_event_slot_availability: Check if an event slot is available
  
  4. Security
    - RLS enabled on all tables
    - Public can create reservations/bookings
...
-- Create event booking status enum type
...
CREATE POLICY "Public can create event bookings"
  ON event_bookings
  FOR INSERT
  TO public
  WITH CHECK (true);
